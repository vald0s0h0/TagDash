// SIC code -> industry title conversion.
// Source: SEC Division of Corporation Finance Standard Industrial Classification (SIC) Code List.
// https://www.sec.gov/corpfin/division-of-corporation-finance-standard-industrial-classification-sic-code-list
// Generated table (444 codes) - do not edit by hand; regenerate from the SEC list.

/// Official SEC SIC code -> industry title pairs, sorted by numeric code for binary search.
static SIC_INDUSTRIES: &[(u32, &str)] = &[
...
/// Convert a SIC code (e.g. "3711") to its English industry title.
/// Accepts surrounding whitespace and leading zeros. Returns None when the
/// code is empty, non-numeric, or not in the official SEC list.
pub fn sic_to_industry(sic: &str) -> Option<&'static str> {
    let target: u32 = sic.trim().parse().ok()?;
    SIC_INDUSTRIES
        .binary_search_by(|(code, _)| code.cmp(&target))
        .ok()
        .map(|i| SIC_INDUSTRIES[i].1)
}
...
mod tests {
    ...
    fn known_codes_resolve() {
        assert_eq!(sic_to_industry("3711"), Some("MOTOR VEHICLES & PASSENGER CAR BODIES"));
        assert_eq!(sic_to_industry("1000"), Some("METAL MINING"));
        assert_eq!(sic_to_industry(" 3711 "), Some("MOTOR VEHICLES & PASSENGER CAR BODIES"));
    }

    #[test]
    fn unknown_or_empty_is_none() {
        assert_eq!(sic_to_industry(""), None);
        assert_eq!(sic_to_industry("0000"), None);
        assert_eq!(sic_to_industry("abcd"), None);
    }

    #[test]
    fn table_is_sorted_for_binary_search() {
        for w in SIC_INDUSTRIES.windows(2) {
