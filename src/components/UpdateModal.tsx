...
export function UpdateModal({ open, onClose }: Props) {
  ...
  // On open: make sure the installed version is known, then run a fresh check so
  // the modal reflects reality without the user clicking anything. Guarded by
  // `busy` inside the store so it won't fight the launch auto-run.
  useEffect(() => {
    if (!open) return;
