// Settings → Données stockées → Dictée vocale. Merges the former Settings "Dictée" tab
// (enable/model/language/pause/jargon config fields) with the former SttModal (model
// download, mic test, job queue) into one tab — previously the jargon list and model
// picker existed in both places independently. This tab is fully self-contained (its own
// config fetch + immediate-apply mutate) rather than going through the parent `draft` +
// global Save button, matching how Apparence/Hotkeys/Stratégies/API Keys already work.
...
export function DicteeTab() {
  ...
  // Poll while mounted (events also refresh, this covers download progress smoothly).
  useEffect(() => {
    refresh();
  ...
  // Seed the jargon editor from config (on mount / when it changes upstream).
  useEffect(() => {
  ...
  // Stop monitoring on unmount (never leave the mic hot).
  useEffect(() => () => {
    monitorRef.current.unlisten?.();
    if (monitoringRef.current) api.sttCancelRecording().catch(() => {});
  }, []);
    ...
    <div className="min-w-0 space-y-3">
      ...
      <div className="flex items-center justify-between gap-2 rounded-md border border-border bg-background/50 px-3 py-2 text-xs">
        <label className="flex items-center gap-2">
          ...
          />
          <span className="text-muted-foreground">Activer la dictée</span>
        </label>
      ...
      <div className="grid grid-cols-2 items-center gap-4">
        <Label className="text-right text-xs text-muted-foreground">Langue (ISO)</Label>
        <Input
      ...
      <div className="grid grid-cols-2 items-center gap-4">
        <Label className="text-right text-xs text-muted-foreground">Pause si CPU &gt; (%)</Label>
        <Input
          type="number"
          value={config.stt.pause_cpu_pct}
          onChange={(e) => patchStt({ pause_cpu_pct: parseFloat(e.target.value) || 0 })}
          className="h-7 text-xs"
        />
      </div>
      <div className="grid grid-cols-2 items-center gap-4">
        <Label className="text-right text-xs text-muted-foreground">Pause après open (min)</Label>
        <Input
          type="number"
          value={config.stt.pause_market_open_minutes}
          onChange={(e) => patchStt({ pause_market_open_minutes: parseInt(e.target.value) || 0 })}
          className="h-7 text-xs"
        />
      </div>
